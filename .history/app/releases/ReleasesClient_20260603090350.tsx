...
/**
 * Fungsi Otomatis untuk Mengubah Link Google Drive Biasa
 * Menjadi Link Gambar Langsung (Direct Image Stream)
 * Menggunakan endpoint thumbnail resolusi tinggi (sz=w1000) untuk performa
 * kilat, aman dari pemblokiran CORS, dan bebas interupsi pemindaian virus.
 */
function formatDriveUrl(url: string): string {
  ...
  // Jika ini link Google Drive, ubah otomatis ke direct download link
  if (cleanUrl.includes("drive.google.com")) {
    const match = cleanUrl.match(/\/d\/([a-zA-Z0-9_-]{25,})/) || cleanUrl.match(/id=([a-zA-Z0-9_-]{25,})/);
    if (match && match[1]) {
      const fileId = match[1];
      // Menggunakan API rendering thumbnail Google Drive resolusi tinggi.
      // Ini sangat stabil, di-cache global oleh CDN Google, dan 100% bebas dari
      // interupsi halaman pemindaian virus untuk file besar.
      return `https://drive.google.com/thumbnail?id=${fileId}&sz=w1000`;
    }
  }
  
  // Kembalikan URL asli jika bukan dari Google Drive (misalnya Unsplash atau hosting lain)
  return cleanUrl;
}
        ...
        {releases.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 w-full">
            {releases.map((release, idx) => {
              // Otomatis convert link cover art di sini sebelum dirender
                ...
                <motion.div key={release.slug || idx} variants={itemFadeUp} className="group relative flex flex-col bg-black/40 backdrop-blur-xl border border-white/5 hover:border-white/20 transition-colors duration-1000 p-6 md:p-8 shadow-[0_30px_60px_rgba(0,0,0,0.5)]">
                  ...
                  <div className="w-full aspect-square relative mb-8 overflow-hidden bg-white/5 border border-white/5">
                    {release.cover ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center text-xs tracking-widest text-white/20 font-mono">
                        NO COVER
                      </div>
                    )}
