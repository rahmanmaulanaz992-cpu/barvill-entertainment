...
export default function ReleasesPage() {
  // Render all artists that have a YouTube Music URL as their catalog gateway
  const activeArtists = artists.filter(artist => artist.youtubeChannelId);
      ...
      >
        ...
        {activeArtists.length === 0 ? (
          <div className="flex items-center justify-center py-32">
            <p className="text-xs tracking-[0.5em] font-mono text-white/30 uppercase">NO ARTISTS AVAILABLE</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 w-full">
            {activeArtists.map((artist, idx) => {
              const coverUrl = artist.image || "/images/fallback-release.jpg";
              
              return (
                <motion.div key={artist.id || idx} variants={itemFadeUp} className="group relative flex flex-col bg-black/40 backdrop-blur-xl border border-white/5 hover:border-white/20 transition-colors duration-1000 p-6 md:p-8 shadow-[0_30px_60px_rgba(0,0,0,0.5)]">
                  <div className="w-full aspect-square relative mb-8 overflow-hidden bg-white/5 border border-white/5">
                    <motion.div className="absolute inset-0 bg-cover bg-center grayscale opacity-70 group-hover:opacity-100 group-hover:scale-[1.05] transition-all duration-[1.5s] ease-[0.16,1,0.3,1]" style={{ backgroundImage: `url('${coverUrl}')` }} />
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_30%,rgba(0,0,0,0.8)_100%)] pointer-events-none mix-blend-multiply opacity-80" />
                  ...
                  <div className="flex flex-col gap-2 mb-8 grow">
                    <h2 className="text-2xl md:text-3xl font-light tracking-[0.1em] uppercase text-white leading-tight mb-2 truncate">{artist.name}</h2>
                    <h3 className="text-sm tracking-[0.2em] uppercase text-white/60 font-medium truncate">{artist.genre?.join(" / ") || "Artist"}</h3>
                  </div>
                  <div className="mt-auto w-full pt-6 border-t border-white/10">
                    <MagneticWrapper>
                      <button onClick={() => window.open(`https://youtube.com/${artist.youtubeChannelId}`, "_blank")} className="relative w-full overflow-hidden group/btn border border-white/10 hover:border-white/30 bg-transparent px-8 py-4 text-[10px] tracking-[0.4em] uppercase text-white cursor-pointer flex items-center justify-center transition-colors duration-700 ease-[0.16,1,0.3,1]">
                        <span className="relative z-10 transition-colors duration-700 ease-[0.16,1,0.3,1] group-hover/btn:text-black font-medium">OPEN YOUTUBE MUSIC</span>
