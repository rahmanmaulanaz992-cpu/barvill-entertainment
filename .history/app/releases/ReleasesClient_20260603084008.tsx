...
// Fungsi sakti untuk mengubah segala jenis link Google Drive menjadi Direct Download Link
function formatDriveUrl(url: string): string {
  if (!url) return "";
  if (url.includes("drive.google.com")) {
    // Pola regex untuk menangkap ID unik file Google Drive
    const match = url.match(/\/d\/([^/]+)/) || url.match(/id=([^&]+)/);
    if (match && match[1]) {
      return `https://docs.google.com/uc?export=download&id=${match[1]}`;
    }
  }
  return url; // Kembalikan URL asli jika bukan link Drive atau link hosting lain
}
        ...
        {releases.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 w-full">
            {releases.map((release, idx) => {
              // Bersihkan link cover milik item rilisan ini secara real-time
              ...
              return (
                <motion.div key={release.slug || idx} variants={itemFadeUp} className="group relative flex flex-col bg-black/40 backdrop-blur-xl border border-white/5 hover:border-white/20 transition-colors duration-1000 p-6 md:p-8 shadow-[0_30px_60px_rgba(0,0,0,0.5)]">
                  ...
                  {/* Container Cover Art Modifikasi Tag Img */}
