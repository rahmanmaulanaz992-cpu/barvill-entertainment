 ...
 * Menggunakan endpoint CDN Google User Content (lh3.googleusercontent.com)
 * yang 100% bebas CORS dan anti-interupsi virus scan.
 */
function formatDriveUrl(url: string): string {
  ...
  // Jika ini link Google Drive, ubah otomatis ke direct CDN link
  if (cleanUrl.includes("drive.google.com")) {
    const match = cleanUrl.match(/\/d\/([a-zA-Z0-9_-]{25,})/) || cleanUrl.match(/id=([a-zA-Z0-9_-]{25,})/);
    if (match && match[1]) {
      const fileId = match[1];
      // Menggunakan endpoint lh3.googleusercontent.com/d/{ID}=w1000
      // Ini adalah endpoint paling stabil untuk menampilkan gambar publik dari Drive di web.
