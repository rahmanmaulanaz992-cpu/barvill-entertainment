...
import { motion, useMotionValue, useSpring, useScroll, useTransform, Variants, useReducedMotion } from "framer-motion";
...
function CinematicDivider() {
  ...
  const lightY = useTransform(scrollYProgress, [0, 1], prefersReducedMotion ? ["0%", "0%"] : ["-30%", "30%"]);
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed inset-0 w-screen h-screen pointer-events-none z-30 flex items-center justify-center transform-gpu will-change-opacity"
        aria-hidden="true"
      >
        ...
        <motion.div
          ...
          className="absolute w-screen h-[40vh] bg-white blur-[60px] md:blur-[120px] rounded-full mix-blend-screen transform-gpu will-change-transform"
        />
      </motion.div>

      <div ref={containerRef} className="relative w-full h-32 md:h-56 flex items-center justify-center z-10 overflow-hidden">
      {/* Core Crisp Line */}
      <motion.div
        ...
        viewport={{ once: true }}
        className="absolute w-full max-w-5xl h-px bg-linear-to-r from-transparent via-white/10 to-transparent origin-center"
      />
...
// Core reusable constants for consistent performance and visual identity
const cinematicEase: [number, number, number, number] = [0.16, 1, 0.3, 1]; // Apple-like smooth cubic-bezier
...
const itemLineYVariants: Variants = {
  ...
  visible: { opacity: 1, scaleY: 1, originY: 0, transition: { duration: 1.8, ease: cinematicEase } },
};

const itemLineXVariants: Variants = {
  hidden: { opacity: 0, scaleX: 0 },
  visible: { opacity: 1, scaleX: 1, transition: { duration: 1.8, ease: cinematicEase } },
};
...
export default function Home() {
  ...
  const artists = [
    { name: "BOYZAI", category: "CINEMATIC HIP-HOP", status: "EXCLUSIVE", cycle: "ACTIVE: 'ECHOES' TOUR" },
    { name: "NOVIELLE", category: "AVANT-GARDE SOUL", status: "NEW SIGNING", cycle: "IN STUDIO: DEBUT ALBUM" },
    { name: "G-MARTYR", category: "INDUSTRIAL EXPERIMENTAL", status: "EXCLUSIVE", cycle: "LATEST: 'THE AWAKENING' VISUAL" },
    { name: "VO-ICE", category: "AMBIENT SOUNDSCAPE", status: "EXCLUSIVE", cycle: "ACTIVE: IMMERSIVE EXHIBITION" },
  // --- CMS READY DATA MOCKUP ---
  const featuredArtist: Artist = {
    name: "BOYZAI",
    category: "CINEMATIC HIP-HOP",
    status: "EXCLUSIVE",
    cycle: "ACTIVE: 'ECHOES' TOUR",
    description: "Sebuah pengalaman audio-visual distopia. Menyatukan tekstur industrial brutal dengan kerentanan introspektif. Karya paling ambisius dari BOYZAI yang mendobrak batasan hip-hop menuju ranah sinematik murni.",
  ...
  const rosterArtists: Artist[] = [
    {
      name: "NOVIELLE",
      category: "AVANT-GARDE SOUL",
      status: "NEW SIGNING",
      cycle: "IN STUDIO: DEBUT ALBUM",
      description: "Eksplorasi vokal ethereal yang melebur dengan arsitektur suara elektronik gelap.",
      image: "https://images.unsplash.com/photo-1516280440502-a279092bf062?auto=format&fit=crop&q=80",
    ...
    },
    {
      ...
      status: "EXCLUSIVE",
      cycle: "LATEST: 'THE AWAKENING' VISUAL",
      description: "Visi murni yang menghancurkan batas antara desain suara mekanis dan emosi mentah.",
      image: "https://images.unsplash.com/photo-1533134486753-c833f0eddebd?auto=format&fit=crop&q=80",
      socials: [{ platform: "SoundCloud", url: "#" }]
    },
    {
      name: "VO-ICE",
      category: "AMBIENT SOUNDSCAPE",
      status: "EXCLUSIVE",
      cycle: "ACTIVE: IMMERSIVE EXHIBITION",
      description: "Arsitek keheningan. Menciptakan ruang hampa yang menggema melalui frekuensi sub-bass.",
  ...
  return (
    <main className="relative w-full bg-black text-white flex flex-col overflow-x-hidden">
      ...
      <motion.div
        animate={prefersReducedMotion ? { opacity: 0.02 } : { x: ["0%", "-1%", "0.5%", "-0.5%", "0%"], y: ["0%", "0.5%", "-1%", "1%", "0%"], opacity: [0.02, 0.04, 0.02] }}
        transition={{
          x: { duration: 15, repeat: Infinity, ease: "linear" },
          y: { duration: 20, repeat: Infinity, ease: "linear" },
          opacity: { duration: 8, repeat: Infinity, ease: "easeInOut" }
        }}
        className="fixed top-[-50%] left-[-50%] w-[200%] h-[200%] pointer-events-none z-40 mix-blend-screen transform-gpu will-change-transform"
      aria-hidden="true"
        style={{
      ...
      <motion.div 
        style={{ y: bgY }}
        className="fixed inset-0 w-full h-screen bg-linear-to-b from-neutral-950 to-black pointer-events-none z-0 transform-gpu" 
        aria-hidden="true"
      />
      ...
      <motion.div className="fixed inset-0 w-full h-screen pointer-events-none z-0 mix-blend-screen" aria-hidden="true">
        {/* Environmental Volumetric Layer 1 */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : envY1 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div
            animate={prefersReducedMotion ? { opacity: 0.02 } : { x: ["-5vw", "10vw", "-5vw"], y: ["-5vh", "5vh", "-5vh"], scale: [1, 1.15, 1], opacity: [0.01, 0.03, 0.01] }}
            transition={{ duration: 45, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-[-20%] left-[-20%] w-[140vw] h-[140vh] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.04)_0%,transparent_60%)] blur-[50px] md:blur-[100px] will-change-transform will-change-opacity"
          />
        </motion.div>

        {/* Environmental Volumetric Layer 2 */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : envY2 }} className="absolute inset-0 w-full h-full pointer-events-none hidden md:block">
          <motion.div
            animate={prefersReducedMotion ? { opacity: 0.02 } : { x: ["10vw", "-10vw", "10vw"], y: ["5vh", "-5vh", "5vh"], scale: [1.1, 0.9, 1.1], opacity: [0.01, 0.04, 0.01] }}
            transition={{ duration: 55, repeat: Infinity, ease: "easeInOut" }}
            className="absolute bottom-[-30%] right-[-20%] w-[150vw] h-[120vh] bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.03)_0%,transparent_60%)] blur-[60px] md:blur-[120px] will-change-transform will-change-opacity"
          />
        </motion.div>

        {/* Environmental Volumetric Layer 3 - Deep Core */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : envY3 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div
      ...
      <motion.div
        style={{ y: prefersReducedMotion ? 0 : glowY1 }}
        animate={prefersReducedMotion ? { opacity: 0.02 } : { x: ["-150vw", "150vw"], opacity: [0.01, 0.04, 0.01] }}
        transition={{
          x: { duration: 35, repeat: Infinity, ease: "linear" },
          opacity: { duration: 15, repeat: Infinity, ease: "easeInOut" }
        }}
      className="fixed top-[-20%] bottom-[-20%] w-screen bg-linear-to-r from-transparent via-white to-transparent blur-[70px] md:blur-[140px] pointer-events-none z-0 mix-blend-screen skew-x-[-30deg] transform-gpu will-change-transform will-change-opacity"
      aria-hidden="true"
      />
      ...
      <motion.div
        ...
        className="fixed top-0 left-0 w-[40vw] aspect-square max-w-2xl rounded-full bg-white/5 blur-[60px] md:blur-[120px] pointer-events-none z-0 mix-blend-screen transform-gpu will-change-transform"
        aria-hidden="true"
      />

      {/* --- BACKGROUND ATMOSPHERE WRAPPER (Parallax) --- */}
      <motion.div className="fixed inset-0 w-full h-screen pointer-events-none z-0" aria-hidden="true">
        ...
        <motion.div style={{ x: bgMouseX, y: bgMouseY }} className="absolute inset-0 w-full h-full pointer-events-none">
          ...
          <motion.div style={{ y: glowY2 }} className="absolute inset-0 w-full h-full pointer-events-none">
            <motion.div className="absolute bottom-[10%] right-[-10%] w-[70vw] h-[50vw] bg-linear-to-l from-white/10 blur-[70px] md:blur-[160px] rounded-full origin-center will-change-transform will-change-opacity hidden md:block"
        ...
        <motion.div style={{ y: prefersReducedMotion ? 0 : glowY2 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div
            animate={prefersReducedMotion ? { opacity: 0.15 } : { x: [0, 80, -40, 0], y: [0, -60, 50, 0], scale: [1, 1.15, 0.9, 1], opacity: [0.1, 0.2, 0.1] }}
            transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-[-15%] left-[-15%] w-[60vw] h-[60vw] rounded-full bg-white/10 blur-[80px] md:blur-[180px] pointer-events-none z-0 mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
        <motion.div style={{ y: prefersReducedMotion ? 0 : glowY1 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div className="absolute bottom-[-15%] right-[-15%] w-[55vw] h-[55vw] rounded-full bg-white/10 blur-[70px] md:blur-[160px] pointer-events-none z-0 mix-blend-screen will-change-transform will-change-opacity hidden md:block"
            animate={prefersReducedMotion ? { opacity: 0.1 } : { x: [0, -90, 50, 0], y: [0, 70, -60, 0], scale: [1, 1.2, 0.95, 1], opacity: [0.08, 0.15, 0.08] }}
            transition={{ duration: 24, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          />
        </motion.div>

      {/* 3. Subtle Radial Light Glow (Animated for Depth) */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : bgY }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div 
            animate={prefersReducedMotion ? { opacity: 0.8 } : { opacity: [0.6, 1, 0.6], scale: [1, 1.05, 1] }}
            transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
            className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.06)_0%,transparent_60%)] pointer-events-none z-0 will-change-opacity will-change-transform" 
          />
        </motion.div>
      ...
      <motion.div style={{ x: prefersReducedMotion ? 0 : midMouseX, y: prefersReducedMotion ? 0 : midMouseY }} className="fixed inset-0 w-full h-screen pointer-events-none z-5 transform-gpu" aria-hidden="true">
        {/* Ambient Blur Layer di Belakang Text */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : midY1 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div
            animate={prefersReducedMotion ? { opacity: 0.05 } : { opacity: [0.03, 0.08, 0.03], scale: [1, 1.1, 1] }}
            transition={{ duration: 20, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[30vh] bg-white/10 blur-[60px] md:blur-[120px] rounded-full mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
        {/* Floating Midground Particles (Lebih tajam & kecil) */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : midY2 }} className="absolute inset-0 w-full h-full pointer-events-none hidden md:block">
          <motion.div 
            animate={prefersReducedMotion ? { opacity: 0 } : { y: ["110vh", "-20vh"], x: [0, 40, -20, 0], opacity: [0, 0.4, 0] }}
            transition={{ duration: 45, repeat: Infinity, ease: "linear" }}
            className="absolute left-[15%] w-1 h-1 bg-white rounded-full blur-[1px] mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
        <motion.div style={{ y: prefersReducedMotion ? 0 : midY1 }} className="absolute inset-0 w-full h-full pointer-events-none hidden md:block">
          <motion.div 
            animate={prefersReducedMotion ? { opacity: 0 } : { y: ["110vh", "-20vh"], x: [0, -30, 20, 0], opacity: [0, 0.3, 0] }}
            transition={{ duration: 60, repeat: Infinity, ease: "linear", delay: 15 }}
            className="absolute left-[75%] w-1.5 h-1.5 bg-white rounded-full blur-[1px] mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
      </motion.div>
      ...
      >
        ...
        <motion.div
          animate={prefersReducedMotion ? {} : { y: [-6, 6, -6], opacity: [0.95, 1, 0.95] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        ...
        >
          ...
          <motion.div variants={itemFadeUpVariants} className="mb-6">
            ...
            >
              A Cinematic Entertainment Universe
            </motion.span>
          </motion.div>
          ...
          <motion.h1
            ...
            className="text-4xl md:text-6xl lg:text-7xl font-light tracking-[0.25em] text-center uppercase leading-tight mb-12"
          >
            BARVILL
            <motion.span 
              animate={prefersReducedMotion ? { opacity: 0.8 } : { opacity: [0.7, 1, 0.7], textShadow: ["0px 0px 0px rgba(255,255,255,0)", "0px 0px 15px rgba(255,255,255,0.2)", "0px 0px 0px rgba(255,255,255,0)"] }}
          ...
          <motion.div
            ...
            className="relative w-px h-16 md:h-24 bg-white/10 mb-12 overflow-hidden"
          >
            <motion.div
              animate={{ y: ["-100%", "300%"] }}
          ...
          <motion.p
            ...
            className="text-xs md:text-sm text-white/60 font-light text-center max-w-xl leading-relaxed italic mb-16 cursor-default"
          >
            &ldquo;Bukan sekadar label. Sebuah universe sinematik di mana frekuensi, visi, dan kemurnian artistik dilebur menjadi warisan abadi.&rdquo;
          </motion.p>
      ...
      <section className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 md:px-6 z-10">
        ...
        >
          ...
          >
            Kami lahir di titik temu antara realitas dan imajinasi. Lebih dari sekadar label, Barvill adalah inkubator bagi para visioner yang menolak berkompromi — memfasilitasi produksi murni, visual sinematik, dan penceritaan immersif yang meredefinisi standar hiburan global.
          </motion.p>
          </motion.div>
        </motion.div>
      </section>
      ...
      {/* --- LATEST RELEASE EDITORIAL SECTION --- */}
      <section className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 md:px-6 py-24 md:py-32 z-10">
        ...
        >
          ...
          <motion.div 
            ...
            role="link"
            tabIndex={0}
            aria-label="View album details: ECHOES"
          ...
          >
            ...
            <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end z-20">
              <div className="flex flex-col">
                ...
                <span className="text-2xl md:text-3xl font-light tracking-[0.2em] text-white">ECHOES</span>
              </div>
              <span className="text-[9px] tracking-[0.3em] text-white/40 border border-white/20 px-4 py-1.5 rounded-full uppercase">Album</span>
            </div>
          </motion.div>
      ...
      <ArtistSpotlight artist={featuredArtist} />
          ...
          <motion.div variants={itemFadeUpVariants} className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <TextReveal as="h2" text="BOYZAI : ECHOES" className="text-3xl md:text-5xl font-light tracking-[0.25em] uppercase mb-8" />
            ...
            <p className="text-sm md:text-base lg:text-lg text-white/60 font-light leading-loose tracking-wider max-w-xl mb-8 cursor-default">
              Sebuah pengalaman audio-visual distopia. Menyatukan tekstur industrial brutal dengan kerentanan introspektif. Karya paling ambisius dari BOYZAI yang mendobrak batasan hip-hop menuju ranah sinematik murni.
            </p>
            <motion.div variants={itemFadeUpVariants} className="flex flex-wrap justify-center lg:justify-start gap-3 md:gap-4 mb-12 w-full max-w-xl">
              <span className="text-[8px] md:text-[9px] tracking-[0.3em] text-white/40 border border-white/10 bg-white/5 px-4 py-2 rounded-full uppercase cursor-default">Digital / Streaming</span>
              <span className="text-[8px] md:text-[9px] tracking-[0.3em] text-white/40 border border-white/10 bg-white/5 px-4 py-2 rounded-full uppercase cursor-default">12&quot; Limited Vinyl</span>
              <span className="text-[8px] md:text-[9px] tracking-[0.3em] text-white/40 border border-white/10 bg-white/5 px-4 py-2 rounded-full uppercase cursor-default">Spatial Audio</span>
            </motion.div>
            <MagneticWrapper>
              <motion.button
                whileHover={{ scale: 1.02, borderColor: "rgba(255,255,255,0.4)", boxShadow: "0px 0px 40px rgba(255,255,255,0.15)" }}
                whileTap={{ scale: 0.95, opacity: 0.8 }}
                transition={{ type: "spring", stiffness: 400, damping: 25 }}
                className="group relative px-10 py-4 border border-white/20 text-white text-[10px] md:text-xs tracking-[0.3em] uppercase overflow-hidden"
              >
      ...
      {/* --- FEATURED VISUAL SECTION (REAL ENTERTAINMENT ECOSYSTEM) --- */}
      <section className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 md:px-6 py-24 md:py-32 z-10">
        <motion.div
          variants={containerVariants}
          initial="hidden"
        ...
        >
          ...
          <div className="flex flex-col items-center text-center w-full max-w-2xl">
            ...
            <TextReveal
              as="h2"
              text="G-MARTYR : THE AWAKENING"
              className="text-3xl md:text-5xl font-light tracking-[0.25em] uppercase mb-8"
            />
            <motion.p
              variants={itemFadeUpVariants}
              className="text-sm md:text-base text-white/60 font-light leading-loose tracking-wider max-w-xl cursor-default"
            >
              Sebuah film pendek distopia. Eksplorasi visual dari lanskap industrial yang hancur, diarahkan oleh visi murni G-MARTYR sebagai pelengkap era album terbarunya.
            </motion.p>
          </div>

          {/* Cinematic 16:9 Video Player Placeholder */}
          <motion.div 
            variants={itemBlurFadeUpVariants} 
            role="button"
            tabIndex={0}
            aria-label="Play visual transmission: G-Martyr The Awakening"
            className="w-full aspect-4/3 md:aspect-video relative group overflow-hidden bg-white/5 border border-white/10 backdrop-blur-md cursor-pointer shrink-0"
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05),transparent_70%)]" />
            <div className="absolute inset-0 bg-black/60 group-hover:bg-black/40 transition-colors duration-1000 ease-out z-10" />
            ...
            <div className="absolute inset-0 flex items-center justify-center z-20">
              <motion.div 
                ...
                whileTap={{ scale: 0.95 }}
                className="w-16 md:w-24 h-16 md:h-24 rounded-full border border-white/20 flex items-center justify-center bg-black/20 backdrop-blur-md transition-all duration-700 group-hover:border-white/50 group-hover:bg-white/10"
              >
                <div className="w-0 h-0 border-y-[6px] border-y-transparent border-l-[10px] border-l-white ml-1 opacity-70 group-hover:opacity-100 transition-opacity duration-500" />
              </motion.div>
            </div>

            {/* Status Tags */}
            <div className="absolute top-6 left-6 md:top-8 md:left-8 z-20 flex gap-3 md:gap-4">
              <span className="text-[8px] md:text-[9px] tracking-[0.3em] text-white/60 bg-black/40 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-full uppercase">Short Film</span>
              <span className="text-[8px] md:text-[9px] tracking-[0.3em] text-white/60 bg-black/40 backdrop-blur-md border border-white/10 px-4 py-1.5 rounded-full uppercase hidden md:block">4K HDR</span>
            </div>
            
            {/* Runtime Indicator */}
            <div className="absolute bottom-6 right-6 md:bottom-8 md:right-8 z-20">
              <span className="text-[10px] tracking-[0.2em] font-light text-white/50 cursor-default">14:23</span>
            </div>
          </motion.div>
        </motion.div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 w-full">
            {artists.map((artist) => (
              <motion.div
                ...
                className="group relative flex flex-col justify-end p-10 md:p-14 min-h-64 md:min-h-72 bg-white/5 border border-white/5 backdrop-blur-md overflow-hidden cursor-pointer"
              >
            ...
            <div className="absolute inset-0 bg-linear-to-br from-transparent to-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-out z-0" />
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-white/10 blur-[50px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-700 ease-out z-0 will-change-opacity" />
                ...
                {/* Hover Explore Action */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-700 ease-out z-20 group-hover:scale-100 scale-95">
                  <span className="text-[10px] tracking-[0.4em] text-white border border-white/20 px-6 py-3 bg-black/40 backdrop-blur-md whitespace-nowrap block">
                    EXPLORE VISION
                  </span>
                </div>

                {/* Real-world Ecosystem Cycle Status */}
                <div className="absolute bottom-8 right-8 text-[8px] tracking-[0.2em] text-white/30 uppercase text-right opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100 z-20">
      ...
      <section className="relative w-full py-16 md:py-24 z-10 overflow-hidden flex items-center justify-center border-y border-white/5">
        ...
        >
          ...
          {[...Array(4)].map((_, idx) => (
            ...
            >
              BARVILL ENTERTAINMENT • THE CINEMATIC UNIVERSE • SOUND DESIGN • VISUAL IDENTITY • IMMERSIVE STORYTELLING •
            </span>
          ))}
        </motion.div>
      </section>
      ...
      <section className="relative w-full min-h-screen flex flex-col items-center justify-center px-4 md:px-6 z-10">
        ...
        >
          {/* Subtle Floating Wrapper */}
          <motion.div
            animate={prefersReducedMotion ? {} : { y: [-4, 4, -4], opacity: [0.98, 1, 0.98] }}
            transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
            className="flex flex-col items-center w-full will-change-transform"
          >
          {/* Subtitle */}
          ...
          >
            Bagi mereka yang menolak kelaziman dan berani mendobrak batas. Mari ciptakan gelombang artistik yang akan terus bergema di masa depan.
          </motion.p>
      ...
      <motion.footer
        ...
        className="relative w-full pt-32 md:pt-48 pb-12 px-4 md:px-6 z-10 flex flex-col items-center bg-linear-to-t from-black"
      >
      ...
      <motion.div style={{ x: prefersReducedMotion ? 0 : fgMouseX, y: prefersReducedMotion ? 0 : fgMouseY }} className="fixed inset-0 w-full h-screen pointer-events-none z-20 hidden md:block transform-gpu" aria-hidden="true">
        {/* Floating Foreground Bokeh (Sangat soft & besar seolah dekat dengan lensa kamera) */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : fgY1 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div 
            animate={prefersReducedMotion ? { opacity: 0 } : { y: ["120vh", "-20vh"], x: [0, 50, -50, 0], opacity: [0, 0.08, 0] }}
            transition={{ duration: 55, repeat: Infinity, ease: "linear", delay: 5 }}
            className="absolute left-[20%] w-10 h-10 bg-white/20 rounded-full blur-md mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
        <motion.div style={{ y: prefersReducedMotion ? 0 : fgY2 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div 
            animate={prefersReducedMotion ? { opacity: 0 } : { y: ["120vh", "-20vh"], x: [0, -70, 40, 0], opacity: [0, 0.06, 0] }}
            transition={{ duration: 70, repeat: Infinity, ease: "linear", delay: 25 }}
            className="absolute left-[80%] w-14 h-14 bg-white/20 rounded-full blur-lg mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
        {/* Atmospheric Light Leak (Cahaya pinggir dari depan layar) */}
        <motion.div style={{ y: prefersReducedMotion ? 0 : fgY1 }} className="absolute inset-0 w-full h-full pointer-events-none">
          <motion.div
            animate={prefersReducedMotion ? { opacity: 0.02 } : { opacity: [0, 0.04, 0], scale: [1, 1.15, 1] }}
            transition={{ duration: 30, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-[-15%] right-[-10%] w-[50vw] h-[50vw] bg-white/10 blur-[80px] md:blur-[150px] rounded-full mix-blend-screen will-change-transform will-change-opacity"
          />
        </motion.div>
      </motion.div>
